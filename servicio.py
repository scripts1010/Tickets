...
import requests, poplib, ConfigParser, sys, urllib3, json, imaplib, email, socket
...
def contentIdDetalle(sessionToken, mail):
    userSessionHeader2 = {'Content-Type': 'text/xml;charset=utf-8','SOAPAction': 'http://archer-tech.com/webservices/ExecuteSearch'} 
    searchOptions = \
    ...
    response = requests.post(baseurl + '/ws/search.asmx', verify=False, data=body2, headers=userSessionHeader2)
    tree2 = ET.fromstring(response.content)
    reg = tree2.find('.//{http://archer-tech.com/webservices/}ExecuteSearchResult').text
...
def verificarAsunto(sessionToken, asunto):
    userSessionHeader2 = {'Content-Type': 'text/xml;charset=utf-8','SOAPAction': 'http://archer-tech.com/webservices/ExecuteSearch'} 
    searchOptions = \
    """<?xml version="1.0" encoding="utf-8"?><SearchReport><DisplayFields><DisplayField>16719</DisplayField><DisplayField>24100</DisplayField></DisplayFields><PageSize>50</PageSize><Criteria><Keywords /><Filter><OperatorLogic /><Conditions><TextFilterCondition><Field>24100</Field><Operator>Contains</Operator><Value>""" + asunto + """</Value></TextFilterCondition></Conditions></Filter><ModuleCriteria><Module>425</Module><IsKeywordModule>True</IsKeywordModule><BuildoutRelationship>Union</BuildoutRelationship><SortFields><SortField><Field>16719</Field><SortType>Ascending</SortType></SortField></SortFields></ModuleCriteria></Criteria></SearchReport>"""
...
def idCliente(sessionToken, idAppCliente,emailCliente):
    userSessionHeader2 = {'Content-Type': 'text/xml;charset=utf-8','SOAPAction': 'http://archer-tech.com/webservices/ExecuteSearch'}
    searchOptions = \
          """<?xml version="1.0" encoding="utf-16"?><SearchReport><DisplayFields><DisplayField>2972</DisplayField></DisplayFields><PageSize>50</PageSize><Criteria><Keywords /><Filter><OperatorLogic /><Conditions><TextFilterCondition><Field>25953</Field><Operator>Contains</Operator><Value>"""+ emailCliente + """</Value></TextFilterCondition></Conditions></Filter><ModuleCriteria><Module>191</Module><IsKeywordModule>True</IsKeywordModule><BuildoutRelationship>Union</BuildoutRelationship><SortFields><SortField><Field>2972</Field><SortType>Ascending</SortType></SortField></SortFields></ModuleCriteria></Criteria></SearchReport>"""
    ...
    tree2 = ET.fromstring(response.content)
    reg = tree2.find('.//{http://archer-tech.com/webservices/}ExecuteSearchResult')
    if reg is None:
       return None
    else:
       reg = reg.text
...
def createJSONTicket(moduleId, msg, mail):
    ...
    if emailmesadeayuda in decola or emailmesadeayuda in paracola or emailmesadeayuda in cccola:
        data['Content']['FieldContents'][str(delacola)]['Value']['ValuesListIds'] = [ticketsIds['mesadeayuda']]
        
    if emailsoporte in decola or emailsoporte in paracola or emailsoporte in cccola:
        data['Content']['FieldContents'][str(delacola)]['Value']['ValuesListIds'] = [ticketsIds['soporte']]
        
    if emailsocclaro in decola or emailsocclaro in paracola or emailsocclaro in cccola:
        data['Content']['FieldContents'][str(delacola)]['Value']['ValuesListIds'] = [ticketsIds['socclaro']] 
...
def deleteMail(msg, servername, email, password):
    ...
    for i in range(tries):
        try:
            ...
            numTry = (-i) + 3
            print("Hubo un error con la conexión al servidor POP de Outlook. Se realizaran " + str(numTry) + " intentos mas para la cuenta " + email)
            logging.error("Hubo un error con la conexión al servidor POP de Outlook. Se realizaran  " + str(numTry) + " intentos mas para la cuenta " + email, exc_info=True)
            if i < tries - 1:
               continue
            else:
               print("No se pudo conectar al servicio POP tras 3 reintentos. Contactarse con un administrador. La cuenta de email es " + email)
               logging.error("No se pudo conectar al servicio POP tras 3 reintentos. Contactarse con un administrador. La cuenta de email es " + email + str(se))
        except poplib.error_proto as pe:
            print("Ocurrio un error con la cuenta "+email+" ¿contraseña erronea?")
            print(pe)
            logging.error("Ocurrio un error con la cuenta "+email+" ¿contraseña erronea?" , exc_info=True)
        break
...
def createAttachments(part):
        ...
        if not part.get_filename():
            name = str(random.randint(1,1000))+ "." + str(part.get_content_type()).split("/")[1]
        ...
        try:
           iD = postAttachment(baseurl, sessionToken, name, data) #Cargo el archivo adjunto en Archer
           ...
           if iD['IsSuccessful']:
              ...
              print(err)
              traceback.print_exc()
              logging.error("Hubo un error al cargar el asunto: " + str(err) , exc_info=True)
              #print iD    
        return value, obs
...
def existeTicketAsociadoAlMensaje(msg, email):
  contentId = None
  ticketId = None
  if 'Ticket#' in decode_header(msg['subject'])[0][0].decode("iso-8859-1"): #El mail posee numero de Ticket
      #Valido que el numero de Ticket sea el de Novared
      if decode_header(msg['subject'])[0][0].decode("iso-8859-1").count("Ticket#") > 1: #En el caso de ANSES tienen un numero de ticket, el primero dentro del Asunto del mail
         contentId = decode_header(msg['subject'])[0][0].decode("iso-8859-1").split("Ticket#")[2]
         contentId = contentId[0:7]
      else:
         contentId = decode_header(msg['subject'])[0][0].decode("iso-8859-1").split("Ticket#")[1]
         contentId = contentId[0:7] #Obtengo el numero de ticket del asunto del mail
...
def actualizarTicket(msg, idTicket, email):
    ...
    for part in msg.walk():
      ...
      if part.get_content_type() == "text/plain":
          ...
          jsonDetalle = createJSONDetalle(body, decode_header(msg['subject'])[0][0].decode("iso-8859-1"), ",".join(getEmails(msg['from'])['emails']), tonames, toemails, valcc, valmailcc, obs)
          jsonDetalleRta = postContent(baseurl, sessionToken, jsonDetalle).json()
          idDetalle = jsonDetalleRta['RequestedObject']['Id']
          jsonTicket = updateJSONTicket(baseurl, idTicket, moduleId, idDetalle)
...
def crearTicket(msg, mail):
    ...
    print emailCliente
    emailId = [{"ContentID":idCliente(sessionToken, clienteTicket, emailCliente)}]
    ...
    if isSuccessful:
       ...
       logging.error("Hubo un error al crear el Ticket Padre. Se copia el JSON de Archer a continuación:")
...
def main(servername, email, password):
    ...
    for msg in messages:
        ...
        try:
         asuntoLogging = decode_header(msg['subject'])[0][0].decode("iso-8859-1")
         logging.error("Asunto del mail: " + asuntoLogging)
         if email == emailsocclaro:
             
             asunto = decode_header(msg['subject'])[0][0].decode("iso-8859-1")
             ...
             if idTicket==None:
                ...
                if isSuccessfulTicket:
                   isSuccessfulDetalle = updateTicket(idTicket, configParser.get('env', 'moduleIdTickets'), msg, email)
             else:
                isSuccessfulTicket = True
                isSuccessfulDetalle = updateTicket(idTicket, configParser.get('env', 'moduleIdTickets'), msg, email)
   
             if isSuccessfulTicket and isSuccessfulDetalle:
                logging.error("Se mueve el mail de carpeta y se elimina del")
                deleteMail(msg, servername, email, password)  
                #deleteMailIMAP(servername, email, password, msgId)            
             else:
                logging.error("Hubo un error al crear el Ticket:" + str(asuntoLogging))
         if email != emailsocclaro:
            ...
            #Se elimina el mail.
            if isSuccessfulTicket and isSuccessfulDetalle:
               logging.error("Se elimina el mail tras una ejecución exitosa")
               deleteMail(msg, servername, email, password)            
            else:
               logging.error("Hubo un error al crear el Ticket:" + str(asuntoLogging))
        except Exception, err:
...
def updateTicket(contentId, moduleId, msg, mail):
    ...
    for part in msg.walk():
        ...
        if part.get_content_type() in allowed_mimetypes:
            ...
            try:
              ...
              if iD['IsSuccessful']:
                 value = iD['RequestedObject']['Id']
                 values.append(value) #En caso que no funcionen los adjuntos, comentar esta línea
              else:
                 obs += iD['ValidationMessages'][0]['ResourcedMessage']
            except Exception, err:
              print err
    ...
    if isSuccessful:
       ...
       logging.error("Hubo un error al crear el Detalle o al actualizar el ticket. Se copia el JSON de Archer a continuación:")
...
allowed_mimetypes = ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/msword', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/pdf', 'text/csv', 'image/png', 'image/jpeg', 'image/gif', 'application/x-zip-compressed', 'text/plain']
